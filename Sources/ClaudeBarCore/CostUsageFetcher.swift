...
public struct CostUsageFetcher: Sendable {
    ...
    public func loadTokenSnapshot(
        ...
        includePiSessions: Bool = false) async throws -> CostUsageTokenSnapshot
    {
        ...
        codexHomePath: String?) -> CostUsageScanner.Options
    {
        _ = provider
        _ = codexHomePath
        return override ?? CostUsageScanner.Options()
    }

    /// App-facing Claude cost snapshot loader (local JSONL logs at API rates).
    public static func loadClaudeTokenSnapshot(
        forceRefresh: Bool = false,
        ...
        retryUnknownPricing: Bool = true) async throws -> CostUsageTokenSnapshot
    {
        ...
        let scanResult = try await CostUsageScanExecutor.run { checkCancellation in
            ...
            if includePiSessions, provider == .claude {
