...
public enum UsageFormatter {
    ...
    public static func creditEventSummary(_ eventDate: Date, service: String, creditsUsed: Double) -> String {
        let formatter = DateFormatter()
        formatter.dateStyle = .medium
        let number = NumberFormatter()
        number.numberStyle = .decimal
        number.maximumFractionDigits = 2
        let credits = number.string(from: NSNumber(value: creditsUsed)) ?? "0"
        return "\(formatter.string(from: eventDate)) · \(service) · \(credits) credits"
    }

    public static func creditEventCompact(_ eventDate: Date, service: String, creditsUsed: Double) -> String {
        let formatter = DateFormatter()
        ...
        number.maximumFractionDigits = 2
        let credits = number.string(from: NSNumber(value: creditsUsed)) ?? "0"
        return "\(formatter.string(from: eventDate)) — \(service): \(credits)"
    }
